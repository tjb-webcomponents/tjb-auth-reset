import WebComponent from"https://tjb-webcomponents.github.io/tjb-webcomponent/tjb-wc.min.js";import html from"https://tjb-webcomponents.github.io/html-template-string/html-template-string.js";import{bounce}from"https://tjb-webcomponents.github.io/tjb-gfx/tjb-gfx.min.js";import"https://tjb-webcomponents.github.io/tjb-input/tjb-input.min.js";import"https://tjb-webcomponents.github.io/tjb-statusbar/tjb-statusbar.min.js";import"https://tjb-webcomponents.github.io/tjb-notify/tjb-notify.min.js";class tjbAuthReset extends WebComponent(){// Styles
////////////////////////////////////////////////////////////
CSS(){return html`
      <style>
        :host {
...
this.handleShowkeyChange=this.reRender}reRender(){super.reRender();if(this.email&&this.showkey){this.messageNode.success=`
        Email send to ${this.email}
      `}}// Logic
////////////////////////////////////////////////////////////
checkValidity(){const toCheck=[];if(this.emailInput)toCheck.push(this.emailInput.checkValidity());if(this.keyInput)toCheck.push(this.keyInput.checkValidity());if(this.passwordInput)toCheck.push(this.passwordInput.checkValidity());return toCheck.every(check=>check)}openHandler(event,target){event.preventDefault();bounce(event.target).then(this._location.bind(this,event.target.href,target))}_location(href,target){if(target==="reset")return this.showkey=false;this.dispatchEvent("redirect",{href,target})}// Async Logic
////////////////////////////////////////////////////////////
fetchHandler(event){event.preventDefault();if(!this.checkValidity())return false;this.statusbar.status="loading";this.email=this.email||this.emailInput.value;const postbody=this.postbody&&this.postbody.replace(/\'/g,"\"");const body=JSON.parse(postbody||"{}");body.email=this.email;body.key=this.keyInput&&this.keyInput.value;body.password=this.passwordInput&&this.passwordInput.value;this.dispatchEvent(this.showkey?"reset":"sendmail",body);if(!this.posturl&&!this.mailurl)return false;return fetch(this.showkey?this.posturl:this.mailurl,{method:"POST",redirect:"follow",credentials:"include",headers:new Headers({"Content-Type":"application/json"}),body:JSON.stringify(body)}).then(resp=>resp.json()).then(resp=>{if(resp.error)throw resp;return this._success(resp)}).catch(resp=>this._error(resp))}_success(resp){console.log("success",resp);this.success().then(e=>{if(!this.showkey)return this.showkey=true;return this.dispatchEvent("success",resp)})}_error(resp){console.error("error",resp);this.dispatchEvent("error",resp);this.error()}success(){return bounce(this.domNode)}error(){return this.errorHandler()}errorHandler(){this.writeMessageError=this.writeMessageError.bind(this);this.statusbar.status="alert";this.domNode.addEventListener("animationend",this.writeMessageError);this.domNode.classList.remove("alert");setTimeout(()=>this.domNode.classList.add("alert"),100)}writeMessageError(){this.domNode.removeEventListener("animationend",this.writeMessageError);this.messageNode.error=!this.showkey?html`
          <ul>
            <li>
              <a
        ...
        `;if(this.emailInput)this.emailInput.showMessage("error");if(this.keyInput)this.keyInput.showMessage("error");if(this.passwordInput)this.passwordInput.showMessage("error")}}customElements.define("tjb-auth-reset",tjbAuthReset);
